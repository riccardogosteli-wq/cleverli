...
import { RECIPIENTS, TEMPLATE, TEMPLATE_HASH, SUBJECT } from '../src/lib/offer149Campaign';
import { execute149, digest149, pages149, material149, type Transport149, type Receipt } from '../src/lib/offer149Transport';
import { NextRequest } from 'next/server';
import { GET, POST } from '../src/app/internal-log-dashboard/offer149/route';
...
function fake() {
  ...
  const io: Transport149 = {
    ...
    async send(payload, key) { assert.equal(payload.subject,SUBJECT); assert.equal(payload.replyTo,'hello@cleverli.ch'); assert.match(key,/^offer149-v3-20260926:/); sent++; return 'provider-fixture'; },
...
test('approved template hash and strict material', () => {
  assert.equal(digest149(TEMPLATE),TEMPLATE_HASH); assert.equal(RECIPIENTS.length,9);
  assert.throws(() => material149(raw,'attacker@example.com'));
  assert.throws(() => material149(raw.replace(RECIPIENTS[0].offerId,'invalid'),email));
  assert.throws(() => material149(JSON.stringify(JSON.parse(raw).slice(1)),email));
  assert.throws(() => material149(raw.replace('"token":','"extra":true,"token":'),email));
});
test('dry run never reserves/sends or returns capability', async () => {
  const f=fake(); const result=await execute149(raw,email,false,false,f.io);
...
test('one success, repeat returns stored receipt without retry', async () => {
  const f=fake(); await execute149(raw,email,true,true,f.io); await execute149(raw,email,true,true,f.io); assert.equal(f.count(),1);
});
test('concurrent reservation sends once', async () => {
  const f=fake(); await Promise.allSettled([execute149(raw,email,true,true,f.io),execute149(raw,email,true,true,f.io)]); assert.equal(f.count(),1);
});
test('changed or suppressed recipient skipped', async () => {
  const f=fake(); f.suppress(); assert.equal((await execute149(raw,email,true,true,f.io)).skipped,true); assert.equal(f.count(),0); assert.equal(await f.io.status(email),null);
});
test('non-production fails before reservation', async () => {
  const f=fake(); await assert.rejects(execute149(raw,email,true,false,f.io)); assert.equal(await f.io.status(email),null);
});
test('provider timeout never retries even without receipt', async () => {
  ...
  await assert.rejects(execute149(raw,email,true,true,f.io));
  assert.equal((await execute149(raw,email,true,true,f.io)).reconciliationRequired,true); assert.equal(attempts,1);
});
test('receipt persistence failure never retries', async () => {
  const f=fake(); f.io.save=async()=>{throw Error('db failure');};
  await assert.rejects(execute149(raw,email,true,true,f.io)); await execute149(raw,email,true,true,f.io); assert.equal(f.count(),1);
});
test('deadline readback mismatch blocks send after irreversible reservation', async () => {
  const f=fake(); f.io.readDeadline=async()=>0; await assert.rejects(execute149(raw,email,true,true,f.io)); assert.equal(f.count(),0); assert.ok(await f.io.status(email));
});
test('fresh eligibility failure after reservation blocks send', async () => {
  const f=fake(); let calls=0; f.io.eligible=async()=>++calls===1; await assert.rejects(execute149(raw,email,true,true,f.io)); assert.equal(f.count(),0);
});
test('pagination traverses later suppression/contact/payment pages, fails closed', async () => {
  const seen=await pages149(async after=>({data:[{id:after?'blocked':'first'}],has_more:!after})); assert.equal(seen[1].id,'blocked');
  await assert.rejects(pages149(async()=>({data:[],has_more:true})));
  await assert.rejects(pages149(async()=>({data:[{id:'same'}],has_more:true})));
});
...
function request(form?: FormData, origin='https://www.cleverli.ch', cookie=signInternalSession()) {
  return new NextRequest('https://www.cleverli.ch/internal-log-dashboard/offer149',{method:form?'POST':'GET',headers:{origin,cookie:`${INTERNAL_LOG_COOKIE}=${cookie}`},...(form?{body:form}:{})});
}
...
test('route rejects duplicate fields and actual oversize bytes',async()=>{
  ...
  assert.equal((await POST(request(form))).status,400);
  const large=new NextRequest('https://www.cleverli.ch/internal-log-dashboard/offer149',{method:'POST',headers:{origin:'https://www.cleverli.ch',cookie:`${INTERNAL_LOG_COOKIE}=${signInternalSession()}`,'content-type':'multipart/form-data; boundary=fixture'},body:'x'.repeat(31000)}); assert.equal((await POST(large)).status,413);
});

import { services149 } from '../src/lib/offer149Server';
function eligibilityFixture(change: string = '') {
 ...
 const deps={services:{db:{from:()=>query,auth:{admin:{getUserById:async()=>({data:{user:{email:change==='email'?'changed@example.com':r.email}},error:null})}}},store:{byId:async()=>({id:r.offerId,user_id:r.userId,customer_id:r.customerId,token_hash:digest149('a'.repeat(64)),amount:14900,currency:'chf',revoked:false,redeemed_session:null})},stripe:{customers:{retrieve:async()=>({id:r.customerId,email:r.email}),list:async()=>({data:[{id:r.customerId}],has_more:false})},subscriptions:{list:change==='subscription'?later({status:'active'}):empty},charges:{list:change==='charge'?later({paid:true,amount:14900}):empty},invoices:{list:change==='invoice'?later({status:'paid',amount_paid:9900}):empty},paymentIntents:{list:change==='payment'?later({status:'succeeded',amount:14900}):empty}}},provider:{suppressions:{list:change==='suppressed'?providerPage(later({email:r.email})):async()=>({data:await empty(),error:null})},contacts:{list:change==='unsubscribed'?providerPage(later({email:r.email,unsubscribed:true})):change==='provider_error'?async()=>({data:null,error:{message:'failed'}}):async()=>({data:await empty(),error:null})}},pace:async()=>{}};
 return services149(deps as unknown as NonNullable<Parameters<typeof services149>[0]>);
}
test('fresh production eligibility succeeds with mocked complete providers',async()=>{
 assert.equal(await eligibilityFixture().io.eligible(RECIPIENTS[0],digest149('a'.repeat(64))),true);
});
for(const changed of ['premium','email','subscription','charge','invoice','payment','suppressed','unsubscribed']) test(`fresh eligibility rejects ${changed}, including later pages`,async()=>{
 assert.equal(await eligibilityFixture(changed).io.eligible(RECIPIENTS[0],digest149('a'.repeat(64))),false);
});
test('provider read failure fails closed',async()=>{
 await assert.rejects(eligibilityFixture('provider_error').io.eligible(RECIPIENTS[0],digest149('a'.repeat(64))));
});
...
for (const event of ['sent','delivered','bounced','complained']) test(`read-only receipt verification distinguishes ${event}`,async()=>{
 ...
 const result=await services149(deps as unknown as NonNullable<Parameters<typeof services149>[0]>).verify(RECIPIENTS[0]);
...
test("missing legacy customer pointer allows verified offer/account/email binding",async()=>{assert.equal(await eligibilityFixture("null_customer").io.eligible(RECIPIENTS[0],digest149("a".repeat(64))),true);});
test("conflicting stored customer pointer stays rejected",async()=>{assert.equal(await eligibilityFixture("conflicting_customer").io.eligible(RECIPIENTS[0],digest149("a".repeat(64))),false);});

import { TEMPLATE as ORIGINAL_TEMPLATE, SUBJECT as ORIGINAL_SUBJECT } from '../src/lib/offer219Campaign';
import { FROM } from '../src/lib/offer149Campaign';
test('only price changes in approved customer copy and sender remains fixed',()=>{
 assert.equal(TEMPLATE,ORIGINAL_TEMPLATE.replaceAll('219','149'));
 assert.equal(SUBJECT,ORIGINAL_SUBJECT.replaceAll('219','149'));
 assert.equal(FROM,'Cleverli <hello@cleverli.ch>');
 assert.equal(RECIPIENTS.some(r=>String(r.email)==='amarverma@hotmail.com'),false);
 assert.throws(()=>material149(raw,'amarverma@hotmail.com'));
});
test('stalled reservation never sends shortened validity',async()=>{
 const f=fake(),reserve=f.io.reserve;
 f.io.reserve=async(...args)=>({...await reserve(...args),claimed_at:new Date(Date.now()-16000).toISOString()});
 await assert.rejects(execute149(raw,email,true,true,f.io));assert.equal(f.count(),0);
});
test('an old campaign ledger entry blocks new campaign eligibility',async()=>{
 const query={select(){return this;},eq(){return this;},async maybeSingle(){return {data:{recipient:email},error:null};}};
 const s=services149({services:{db:{from:()=>query}},provider:{},pace:async()=>{}} as unknown as NonNullable<Parameters<typeof services149>[0]>);
 assert.equal(await s.io.eligible(RECIPIENTS[0],'hash'),false);
});
