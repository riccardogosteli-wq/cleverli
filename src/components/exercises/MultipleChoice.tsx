      ...
      <div className="space-y-4">
        ...
        <div className={`grid gap-3 ${options.length <= 2 ? "grid-cols-2" : "grid-cols-2"}`}>
          {options.map((opt, i) => {
            ...
            let bg = "#ffffff";
            if (submitted && isSelected && isCorrect) { borderColor = "#22c55e"; bg = "#f0fdf4"; }
            else if (submitted && isSelected && !isCorrect) { borderColor = "#ef4444"; bg = "#fef2f2"; }
            else if (submitted && selected && isCorrect) { borderColor = "#22c55e"; bg = "#f0fdf4"; }
            else if (isSelected) { borderColor = "#16a34a"; bg = "#f0fdf4"; }
            ...
            return (
              <button
                ...
                style={{
                  borderColor,
                  background: bg,
                  animation: submitted && isSelected && !isCorrect && shake ? "shake 0.4s ease" : undefined,
                  minHeight: "110px",
                }}
              >
                ...
                {submitted && !isSelected && selected && isCorrect && (
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3">
        {options.map((opt, i) => {
          ...
          if (submitted && isSelected && isCorrect) { cls += "bg-green-100 border-green-500 text-green-800 shadow-md"; style.transform = "scale(1.03)"; }
          else if (submitted && isSelected && !isCorrect) { cls += "bg-red-100 border-red-400 text-red-700"; style.animation = shake ? "shake 0.4s ease" : undefined; }
          else if (submitted && selected && isCorrect) { cls += "bg-green-100 border-green-400 text-green-800"; style.transform = "scale(1.02)"; }
          else if (isSelected) { cls += "bg-green-50 border-green-500 text-green-800 shadow-sm"; }
          else { cls += "bg-white border-gray-200 hover:border-green-400 hover:bg-green-50"; }
          const badge = submitted && isSelected && isCorrect ? "✓" : submitted && isSelected && !isCorrect ? "✗" : submitted && selected && isCorrect ? "✓" : `${i + 1}`;
          const badgeColor = submitted && isSelected && isCorrect ? "bg-green-500 text-white" : submitted && isSelected && !isCorrect ? "bg-red-400 text-white" : submitted && selected && isCorrect ? "bg-green-500 text-white" : isSelected ? "bg-green-500 text-white" : "bg-gray-100 text-gray-500";

          return (
            <button key={opt} onClick={() => pick(opt)} disabled={submitted} aria-pressed={isSelected} data-answer={opt} className={cls} style={style}>
