...
import { verifyLisaProvider, LISA_EMAIL } from '@/lib/outreachLisaVerification';
...
export async function POST(req: NextRequest) {
  ...
  if (action !== 'preview' && action !== 'send' && action !== 'verify-lisa') return NextResponse.json({ error: 'invalid_action' }, { status: 400, headers });
  if (action === 'send' && form.get('confirmed') !== 'yes') return NextResponse.json({ error: 'confirmation_required' }, { status: 400, headers });
  try {
    if (action === 'verify-lisa') {
      if (email !== LISA_EMAIL) return NextResponse.json({error:'invalid_recipient'}, {status:400,headers});
      try { return NextResponse.json(await verifyLisaProvider(), {headers}); }
