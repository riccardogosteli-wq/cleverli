...
import { outreachRecipient, outreachContent, ALL_OUTREACH_RECIPIENTS as OUTREACH_RECIPIENTS, OUTREACH_SUBJECT, OUTREACH_FROM, OUTREACH_REPLY_TO, OUTREACH_CAMPAIGN } from '@/lib/outreachCampaign';
import { verifyLisaProvider, LISA_EMAIL, VERIFIED_OUTREACH_EMAILS } from '@/lib/outreachLisaVerification';
...
export async function POST(req: NextRequest) {
  ...
  if (action !== 'preview' && action !== 'send' && action !== 'verify-lisa' && action !== 'verify-provider') return NextResponse.json({ error: 'invalid_action' }, { status: 400, headers });
  if (action === 'send' && form.get('confirmed') !== 'yes') return NextResponse.json({ error: 'confirmation_required' }, { status: 400, headers });
  try {
    if (action === 'verify-lisa' || action === 'verify-provider') {
      if (!VERIFIED_OUTREACH_EMAILS.some(e => e === email) || (action === 'verify-lisa' && email !== LISA_EMAIL)) return NextResponse.json({error:'invalid_recipient'}, {status:400,headers});
      try { return NextResponse.json(await verifyLisaProvider(email), {headers}); }
      catch(error) { const reason=error instanceof Error && /^provider_[a-z_]+(?:_[0-9]+)?$/.test(error.message) ? error.message : 'provider_verification_unavailable'; return NextResponse.json({error:reason,noEmailSent:true}, {status:503,headers}); }
    }
    if (action === 'preview') {
