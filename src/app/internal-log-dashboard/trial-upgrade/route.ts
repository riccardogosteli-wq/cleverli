...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'send' && !threeDayTrialWindow(Math.floor(Date.now()/1000))) return json({ error: 'three_day_trial_window_requires_review', instruction: 'V3 promises three days, but the approved trial ends sooner. No reservation or email was made.' }, 409);
