...
import { FROM, SUBJECT } from '@/lib/offer219Campaign';
import { executeTrialMail, TRIAL_TEMPLATE } from '@/lib/trialUpgradeMail';
import { TRIAL_UPGRADE, threeDayTrialWindow } from '@/lib/trialUpgrade';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'send' && !threeDayTrialWindow(Math.floor(Date.now()/1000))) return json({ error: 'three_day_trial_window_requires_review', instruction: 'Dispatch requires all three days within the trial and a safe final daily 15:00 UTC preparation window. This request made no new reservation and sent no email.' }, 409);
