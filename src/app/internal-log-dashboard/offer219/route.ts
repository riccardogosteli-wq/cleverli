...
import { FROM, RECIPIENTS, SUBJECT, TEMPLATE } from '@/lib/offer219Campaign';
import { execute219 } from '@/lib/offer219Transport';
import { services219 } from '@/lib/offer219Server';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'verify') {
      ...
      return json(await services219().verify(recipient));
    }
    ...
    return json(await execute219(await file.text(), recipient.email, action === 'send', process.env.VERCEL_ENV === 'production', services219().io));
