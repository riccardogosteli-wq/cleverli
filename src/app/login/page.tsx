"use client";
import { useState } from "react";
import Link from "next/link";
import CleverliMascot from "@/components/CleverliMascot";
import { useLang } from "@/lib/LangContext";

export default function Login() {
  const { tr } = useLang();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");

  return (
    <div className="min-h-screen bg-green-50 flex flex-col items-center justify-start pt-8 px-4 pb-16">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center">
          <CleverliMascot size={100} />
          <h1 className="mt-4 text-2xl font-bold text-gray-900">{tr("login")}</h1>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 space-y-3">
          <input type="email" value={email} onChange={e => setEmail(e.target.value)}
            placeholder={tr("emailPlaceholder")}
            className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 outline-none focus:border-green-500 bg-white text-sm transition-colors" />
          <input type="password" value={password} onChange={e => setPassword(e.target.value)}
            placeholder={tr("passwordPlaceholder")}
            className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 outline-none focus:border-green-500 bg-white text-sm transition-colors" />
          <button className="w-full bg-green-600 text-white py-3 rounded-xl font-semibold hover:bg-green-700 transition-colors text-sm">
            {tr("login")}
          </button>
          <div className="flex items-center gap-3">
            <div className="flex-1 h-px bg-gray-200" />
            <span className="text-xs text-gray-400">oder</span>
            <div className="flex-1 h-px bg-gray-200" />
          </div>
          <button className="w-full border-2 border-gray-200 bg-white py-3 rounded-xl font-semibold text-gray-700 hover:bg-gray-50 text-sm flex items-center justify-center gap-2 transition-colors">
            <span>🔵</span> Weiter mit Google
          </button>
        </div>
        <p className="text-center text-sm text-gray-600">
          {tr("noAccount")}{" "}
          <Link href="/signup" className="text-green-600 font-semibold hover:underline">{tr("signup")}</Link>
        </p>
      </div>
    </div>
  );
}
