...
import { CAMPAIGN, FROM, RECIPIENTS, SUBJECT } from './offer149Campaign';
import { pages149, type Recipient, type Receipt, type Transport149 } from './offer149Transport';

export function services149(dependencies?: { services: ReturnType<typeof privateOfferServices>; provider: Resend; pace: () => Promise<unknown> }) {
  ...
  async function contacts() {
    return pages149(async after => { await pace(); const r = await resend.contacts.list({ limit: 100, after }); if (r.error || !r.data) throw Error('provider_check_failed'); return r.data; });
  }
  async function suppressions() {
    return pages149(async after => { await pace(); const r = await resend.suppressions.list({ limit: 100, after }); if (r.error || !r.data) throw Error('provider_check_failed'); return r.data; });
  }
  const io: Transport149 = {
    async eligible(r, hash) {
      for (const table of ['offer219_mail', 'trial_upgrade_mail']) {
        const prior = await db.from(table).select('recipient').eq('recipient', r.email).maybeSingle();
        if (prior.error) throw Error('history_unavailable');
        if (prior.data) return false;
      }
      const offer = await store.byId(r.offerId);
      if (!offer || offer.token_hash !== hash || offer.user_id !== r.userId || offer.customer_id !== r.customerId || offer.amount !== 14900 || offer.currency !== 'chf' || offer.revoked || offer.redeemed_session) return false;
      ...
      const customers = await pages149(after => stripe.customers.list({ email: r.email, limit: 100, starting_after: after }));
      if (!customers.some(c => c.id === r.customerId)) return false;
      for (const c of customers) {
        const subscriptions = await pages149(after => stripe.subscriptions.list({ customer: c.id, status: 'all', limit: 100, starting_after: after }));
        if (subscriptions.some(s => !['canceled', 'incomplete_expired'].includes(s.status))) return false;
        const charges = await pages149(after => stripe.charges.list({ customer: c.id, limit: 100, starting_after: after }));
        if (charges.some(c => c.paid && c.amount > 0)) return false;
        const invoices = await pages149(after => stripe.invoices.list({ customer: c.id, limit: 100, starting_after: after }));
        if (invoices.some(i => i.status === 'paid' && i.amount_paid > 0)) return false;
        const payments = await pages149(after => stripe.paymentIntents.list({ customer: c.id, limit: 100, starting_after: after }));
    ...
    async status(email) {
      const r = await db.from('offer149_mail').select('recipient,state,provider_id,deadline,claimed_at,body_hash').eq('campaign', CAMPAIGN).eq('recipient', email).maybeSingle();
      if (r.error) throw Error('ledger_unavailable'); return r.data as Receipt | null;
    },
    async reserve(r, hash, bodyHash) {
      const result = await db.rpc('reserve_offer149_mail', { p_recipient: r.email, p_offer: r.offerId, p_user: r.userId, p_customer: r.customerId, p_hash: hash, p_body: bodyHash });
    ...
    async save(email, id) {
      const r = await db.from('offer149_mail').update({ provider_id: id, state: 'accepted', accepted_at: new Date().toISOString() }).eq('campaign', CAMPAIGN).eq('recipient', email).eq('state', 'reserved').is('provider_id', null).select('provider_id').single();
  ...
  async function verify(r: Recipient) {
    ...
    const emails = await pages149(async after => { await pace(); const r = await resend.emails.list({ limit: 100, after }); if (r.error || !r.data) throw Error('provider_verification_failed'); return r.data; });
