...
import { CAMPAIGN, FROM, RECIPIENTS, SUBJECT, TEMPLATE } from './offer219Campaign';
export type Recipient = typeof RECIPIENTS[number];
export const digest219 = (s: string) => createHash('sha256').update(s).digest('hex');
...
export function material219(raw: string, email: string) {
  const r = RECIPIENTS.find(r => r.email === email);
  if (!r || raw.length > 24000) throw Error('invalid_material');
  const rows: unknown = JSON.parse(raw);
  if (!Array.isArray(rows) || rows.length !== 20) throw Error('invalid_material');
  ...
  return { r, hash: digest219(token), html, bodyHash: digest219(html) };
}
export async function execute219(raw: string, email: string, send: boolean, production: boolean, io: Transport219) {
  if (send && !production) throw Error('production_only');
  const m = material219(raw, email);
...
export async function pages219<T extends { id: string }>(load: (after?: string) => Promise<{ data: T[]; has_more: boolean }>): Promise<T[]> {
