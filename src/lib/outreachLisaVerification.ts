...
export async function verifyLisaProvider() {
 ...
 for (const kind of ['emails','suppressions'] as const) {
  ...
  for (let page=0;page<100;page++) {
   ...
   if(!result.data || !Array.isArray(result.data.data)) throw Error('provider_read_unavailable');
   const rows=result.data.data;
   scanned+=rows.length;
   for(const row of rows) {
    if(kind==='emails' && 'to' in row && [...row.to,...(row.cc||[]),...(row.bcc||[])].some(e=>e.toLowerCase()===LISA_EMAIL)) history.push({id:row.id,to:row.to,subject:row.subject,last_event:row.last_event});
    if(kind==='suppressions' && 'email' in row && row.email.toLowerCase()===LISA_EMAIL) suppressions.push({email:row.email,origin:row.origin});
   }
 ...
 const row=ledger.find(r=>r.email===LISA_EMAIL);
 let receipt=null;
 if(row?.provider_id) {
  ...
  const r=result.data;
  if(r.to.length!==1||r.to[0].toLowerCase()!==LISA_EMAIL)throw Error('provider_recipient_mismatch');
 ...
 return {email:LISA_EMAIL,checkedAt:new Date().toISOString(),complete:true,scanned,history,suppressions,clear:history.length===0&&suppressions.length===0,receipt};
}
