...
function mc(id: string, difficulty: 1 | 2 | 3, item: ListeningCase, free = false, speakChoices = false): Exercise {
  ...
  const listeningText = optionEmojis || optionImages || speakChoices
    ? `${item.audio} ${item.question[0]} ${options.map((option, index) => `Antwort ${index + 1}: ${option.replace(/[.!?]+$/u, "")}`).join(". ")}.`
    : item.audio;
  return {
    id,
...
function order(id: string, difficulty: 2 | 3, audio: string, steps: [string, string, string], emojis?: [string, string, string]): Exercise {
  ...
  return {
    ...
    listeningText: emojis ? `${audio} Ordne die drei Bilder so, wie du es gehört hast.` : audio,
    answer: "all",
    hints: [hint1[0], hint2[0]],
    hintsEN: [hint1[1], hint2[1]],
    hintsFR: [hint1[2], hint2[2]],
...
const grade1Easy: ListeningCase[] = [
  { audio: "Das Kind sieht den Ball. Der Ball ist rot.", question: q("Was sieht das Kind?", "What does the child see?", "Que voit l'enfant ?", "Cosa vede il bambino?"), answer: "roter Ball", wrong: ["blaue Tasche", "grüner Frosch", "gelbes Buch"], images: ["/images/listening/ball-red.svg", "/images/listening/bag-blue.svg", "/images/listening/frog-green.svg", "/images/listening/book-yellow.svg"] },
  ...
  { audio: "Der Hund schläft im Korb.", question: q("Wo schläft der Hund?", "Where is the dog sleeping?", "Où dort le chien ?", "Dove dorme il cane?"), answer: "im Korb", wrong: ["unter dem Tisch", "auf dem Sofa", "im Garten"], visuals: ["🧺", "🪑", "🛋️", "🌳"] },
  ...
  { audio: "Ben hat drei blaue Stifte.", question: q("Wie viele Stifte hat Ben?", "How many pencils does Ben have?", "Combien de crayons Ben a-t-il ?", "Quante matite ha Ben?"), answer: "drei", wrong: ["zwei", "vier", "fünf"], visuals: ["3️⃣", "2️⃣", "4️⃣", "5️⃣"] },
  ...
  { audio: "Auf dem Tisch liegen Bücher.", question: q("Was liegt auf dem Tisch?", "What is on the table?", "Qu'y a-t-il sur la table ?", "Cosa c'è sul tavolo?"), answer: "Bücher", wrong: ["Hefte", "ein Ball", "Äpfel"], visuals: ["📚", "📒", "⚽", "🍎"] },
...
const grade1Medium: ListeningCase[] = [
  { audio: "Zieh zuerst die Jacke an. Nimm dann deine Tasche.", question: q("Was soll das Kind zuerst tun?", "What should the child do first?", "Que doit faire l'enfant d'abord ?", "Cosa deve fare prima il bambino?"), answer: "die Jacke anziehen", wrong: ["die Tasche nehmen", "die Schuhe ausziehen", "das Fenster öffnen"], visuals: ["🧥", "🎒", "👟", "🪟"] },
  { audio: "Stell den Becher auf den Tisch, direkt neben den Teller.", question: q("Neben was gehört der Becher?", "What should the cup go next to?", "À côté de quoi faut-il mettre le gobelet ?", "Accanto a cosa va messo il bicchiere?"), answer: "neben den Teller", wrong: ["neben den Stuhl", "neben die Tasche", "neben die Tür"], visuals: ["🍽️", "🪑", "🎒", "🚪"] },
  ...
  { audio: "Mia sucht ihren Ball. Er liegt unter dem Bett.", question: q("Wo findet Mia den Ball?", "Where does Mia find the ball?", "Où Mia trouve-t-elle le ballon ?", "Dove trova Mia la palla?"), answer: "unter dem Bett", wrong: ["unter dem Stuhl", "im Korb", "vor der Haustür"], visuals: ["🛏️", "🪑", "🧺", "🏠"] },
  ...
  { audio: "Es ist kalt. Nina zieht Handschuhe und eine Mütze an.", question: q("Wie ist das Wetter?", "What is the weather like?", "Quel temps fait-il ?", "Com'è il tempo?"), answer: "kalt", wrong: ["heiss", "windig", "regnerisch"], visuals: ["🥶", "🥵", "🌬️", "🌧️"] },
  ...
  { audio: "Auf dem Pausenplatz spielt Sara mit Mia und Tom.", question: q("Wie viele Kinder spielen mit Sara?", "How many children are playing with Sara?", "Combien d'enfants jouent avec Sara ?", "Quanti bambini giocano con Sara?"), answer: "zwei", wrong: ["eins", "drei", "vier"], visuals: ["2️⃣", "1️⃣", "3️⃣", "4️⃣"] },
...
const grade1Hard: ListeningCase[] = [
  ...
  { audio: "Ben hört es donnern. Er sieht dunkle Wolken. Schnell schliesst er das Fenster.", question: q("Was kommt wahrscheinlich?", "What will probably happen?", "Que va-t-il probablement se passer ?", "Cosa succederà probabilmente?"), answer: "Gewitter", wrong: ["Sonne", "Schnee", "Nebel"], visuals: ["⛈️", "☀️", "🌨️", "🌫️"] },
  ...
  { audio: "Sara giesst die Erde jeden Tag. Nach einer Woche schaut ein grüner Stängel aus dem Topf.", question: q("Was wächst im Topf?", "What is growing in the pot?", "Qu'est-ce qui pousse dans le pot ?", "Cosa cresce nel vaso?"), answer: "eine Pflanze", wrong: ["ein Stein", "ein Pilz", "ein Spielzeug"], visuals: ["🌱", "🪨", "🍄", "🧸"] },
  ...
  { audio: "Ben möchte den hohen Ast erreichen. Er stellt sich nicht auf den wackligen Stuhl, sondern bittet einen Erwachsenen um Hilfe.", question: q("Was macht Ben?", "What does Ben do?", "Que fait Ben ?", "Cosa fa Ben?"), answer: "Er bittet einen Erwachsenen um Hilfe.", wrong: ["Er klettert auf den Stuhl.", "Er springt nach dem Ast.", "Er zieht am Baum."], visuals: ["🧑", "🪑", "🤸", "🌳"] },
  ...
  { audio: "Amir teilt seine vier Farbstifte mit Lio. Nun können beide gleichzeitig malen.", question: q("Was können beide Kinder nun tun?", "What can both children do now?", "Que peuvent maintenant faire les deux enfants ?", "Cosa possono fare ora entrambi i bambini?"), answer: "malen", wrong: ["schlafen", "lesen", "essen"], visuals: ["🎨", "💤", "📖", "🍽️"] },
  { audio: "Lea hört die Schulglocke, steckt das Springseil ein und läuft mit den anderen Kindern zur Tür.", question: q("Was beginnt wahrscheinlich?", "What is probably beginning?", "Qu'est-ce qui commence probablement ?", "Cosa sta probabilmente iniziando?"), answer: "der Unterricht", wrong: ["das Abendessen", "die Schlafenszeit", "das Schwimmen"], visuals: ["🏫", "🍽️", "🛏️", "🏊"] },
];
...
const grade1Orders: Array<[string, [string, string, string], [string, string, string]]> = [
  ["Zuerst klingelt Bens Wecker. Dann isst er ein Brot. Zum Schluss geht er zur Schule.", ["Bens Wecker klingelt.", "Ben isst ein Brot.", "Ben geht zur Schule."], ["⏰", "🍞", "🏫"]],
  ["Zuerst nimmt Mia einen Samen. Dann giesst sie ihn. Später wächst eine Blume.", ["Mia nimmt einen Samen.", "Mia giesst den Samen.", "Eine Blume wächst."], ["🫘", "💧", "🌻"]],
  ["Zuerst nimmt Noah eine Schüssel. Dann kommt der Teig in den Ofen. Zuletzt ist der Kuchen fertig.", ["Noah nimmt eine Schüssel.", "Der Teig kommt in den Ofen.", "Der Kuchen ist fertig."], ["🥣", "♨️", "🎂"]],
  ["Zuerst zieht Sara die Jacke an. Dann nimmt sie die Schuhe. Zuletzt geht sie zur Tür.", ["Sara zieht die Jacke an.", "Sara nimmt die Schuhe.", "Sara geht zur Tür."], ["🧥", "👟", "🚪"]],
  ["Zuerst nimmt Lea ein Buch. Dann liest sie. Zuletzt stellt sie das Buch ins Regal.", ["Lea nimmt ein Buch.", "Lea liest.", "Lea stellt das Buch ins Regal."], ["📕", "📖", "📚"]],
];
