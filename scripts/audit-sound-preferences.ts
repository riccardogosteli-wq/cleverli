...
for (const lang of ["de","fr","it","en"] as const) for (const key of ["title","effects","autoRead","note","on","off","button","close"] as const) check(Boolean(SOUND_COPY[lang][key]),true);
...
check(player.includes('voiceOn'),false);check(player.includes('<ExerciseSoundSettings />'),true);check(player.includes('useAutomaticReading('),true);
...
check(player.includes('<SoundPreferences />'),false);
const ui=fs.readFileSync('src/components/SoundPreferences.tsx','utf8');
check(ui.includes('EffectsToggle'),false);check(ui.includes('showModal()'),true);check(ui.includes('aria-haspopup="dialog"'),true);check(ui.includes('onClose='),true);check(ui.includes('trigger.current?.focus'),true);
