...
const campaign=load('src/lib/outreachCampaign.ts',{});let state='ready',fail=false,saveFail=false;
...
(async()=>{
 assert.equal(campaign.OUTREACH_RECIPIENTS.length,5);checks++;
 ...
 const page=await route.GET(req({method:'GET'}));assert.equal(page.status,200);assert.equal((await page.text()).match(/<option /g).length,5);checks+=2;
