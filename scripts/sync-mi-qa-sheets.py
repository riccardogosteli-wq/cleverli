...
def sync_grade(grade: int, apply: bool) -> dict:
    ...
    mi_rows = [row for row in source["rows"] if row.get("subjectId") == "mi"]
    mi_special_rows = [row for row in mi_rows if row["type"] in SPECIAL_TYPES]
    ...
    missing_main = [row for row in mi_rows if row["exerciseId"] not in existing_main_ids]
    missing_special = [row for row in mi_special_rows if row["exerciseId"] not in existing_special_ids]
    ...
    missing_after_main = [row["exerciseId"] for row in mi_rows if row["exerciseId"] not in reread_main_ids]
    missing_after_special = [row["exerciseId"] for row in mi_special_rows if row["exerciseId"] not in reread_special_ids]

    return {
        "miRows": len(mi_rows),
        "miSpecialRows": len(mi_special_rows),
...
def main() -> None:
    ...
    if any(result["missingAfterMain"] or result["missingAfterSpecial"] for result in summary.values()):
        raise SystemExit(1)
