...
def is_language_rich(row: dict, grade: int) -> bool:
    subject_id = row.get("subjectId")
    exercise_id = str(row.get("exerciseId", ""))
    return (
        subject_id in {"english", "french"}
        and (exercise_id.startswith(f"eng{grade}-rich-") or exercise_id.startswith(f"fr{grade}-rich-"))
    )


def sync_grade(grade: int, apply: bool) -> dict:
    source = json.loads(Path(f"/tmp/cleverli-grade-{grade}-qa.json").read_text())
    rows = [row for row in source["rows"] if is_language_rich(row, grade)]
    ...
    return {
        "languageRichRows": len(rows),
        "languageRichSpecialRows": len(special_rows),
