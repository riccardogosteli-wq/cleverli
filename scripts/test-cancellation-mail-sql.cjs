...
(async()=>{
 ...
 create table public.trial_upgrade_offers(subscription_id text);
 ...
 const activationBefore=JSON.stringify((await db.query('select * from cancellation_mail_activation')).rows);
 await db.exec(fs.readFileSync('supabase/2026-09-21-cancellation-mail-upgrade-guard.sql','utf8'));
 await db.exec(fs.readFileSync('supabase/2026-09-21-cancellation-mail-readiness.sql','utf8'));
 assert.equal(JSON.stringify((await db.query('select * from cancellation_mail_activation')).rows),activationBefore);ok();
 ...
 await db.exec("insert into trial_upgrade_offers values('sub_fixture')");await assert.rejects(enqueue(),/trial_upgrade_excluded/);await db.exec('delete from trial_upgrade_offers');ok();
 await db.exec("insert into trial_upgrade_offers values('sub_unrelated')");
 ...
 assert.equal(r.ready_at,null);assert.equal((await claim()).id,null);ok();
 assert.equal((await db.query('select first_attempt_at from cancellation_mail_outbox where id=$1',[r.id])).rows[0].first_attempt_at,null);ok();
 const ready=async(id)=>(await db.query('select ready_cancellation_mail($1) as ok',[id])).rows[0].ok;
 await db.exec("update parent_profiles set premium_plan='schooltime'");assert.equal(await ready(r.id),false);assert.equal((await claim()).id,null);await db.exec("update parent_profiles set premium_plan='monthly'");ok();
 assert.equal(await ready(r.id),true);const readyAt=(await db.query('select ready_at from cancellation_mail_outbox where id=$1',[r.id])).rows[0].ready_at;assert.ok(readyAt);ok();
 assert.equal(await ready(r.id),true);assert.equal(new Date((await db.query('select ready_at from cancellation_mail_outbox where id=$1',[r.id])).rows[0].ready_at).getTime(),new Date(readyAt).getTime());ok();
 ...
 const receiptBefore=JSON.stringify((await db.query('select * from cancellation_mail_outbox where id=$1',[r.id])).rows);
 await db.exec(fs.readFileSync('supabase/2026-09-21-cancellation-mail-readiness.sql','utf8'));
 assert.equal(JSON.stringify((await db.query('select * from cancellation_mail_outbox where id=$1',[r.id])).rows),receiptBefore);ok();
 // A new cycle is distinct; ambiguous work older than the provider window is frozen.
 const r2=(await enqueue('evt_cycle2',fence+1)).rows[0];assert.notEqual(r2.id,r.id);ok();assert.equal(await ready(r2.id),true);
 ...
 const r3=(await enqueue('evt_cycle3',fence+2)).rows[0];await ready(r3.id);await db.query("update cancellation_mail_outbox set end_at=clock_timestamp()-interval '1 second' where id=$1",[r3.id]);
 ...
 for(const role of ['anon','authenticated']){
  ...
  await assert.rejects(db.exec(`set role ${role};select ready_cancellation_mail('${r.id}')`),/permission denied/);await db.exec('reset role');ok();
 }
 const r4=(await enqueue('evt_cycle4',fence+3)).rows[0];await ready(r4.id);const firstLease=await claim(r4.id);
 ...
 const r5=(await enqueue('evt_cycle5',fence+4)).rows[0];await ready(r5.id);await db.exec("insert into trial_upgrade_offers values('sub_fixture')");
 assert.equal((await claim(r5.id)).id,null);assert.equal((await db.query('select state from cancellation_mail_outbox where id=$1',[r5.id])).rows[0].state,'suppressed');ok();
